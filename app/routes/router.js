...
router.put(
  '/v1/pay/updatepay/:id',
  uploadOnMemory.single('image'),
  transaction.handlepaymentbookingid,
)
router.put(
  '/v1/pay/updatepay/:bookingId',
  uploadOnMemory.single('image'),
...
// flight
router.get(
  '/v1/flights/search?:depDate?:depAirport?:arrAirport?',
  flightController.handleSearchFlight,
)
...
router.put(
  ...
  authorize,
  isAdmin,
  flightController.handleUpdateFlight,
)
router.delete(
  '/v1/flights/:id',
  authorize,
  isAdmin,
  flightController.handleDeleteFlight,
)
...
router.post(
  ...
  authController.handleRegisterGoogle,
...
router.get(
  ...
  authorize,
  isAdmin,
  bookingController.handleListBookings,
)
router.get('/v1/bookings', authorize, bookingController.handleGetUserBooking)
router.get(
  '/v1/bookings/search?:bookingcode?',
  bookingController.handleSearchBookingByCode,
)
...
router.get(
  ...
  authorize,
  isAdmin,
  transaction.getBookingTransaction,
)
router.get(
  '/v1/pay/find/all',
  authorize,
...
router.put(
  ...
  authorize,
  isAdmin,
  transaction.handleConfirmPayment,
)
router.put(
  '/v1/pay/reject/:id',
  authorize,
  isAdmin,
  transaction.handleRejectPayment,
)

// by id
router.get('/v1/pay/transaction/:id', authorize, isAdmin, transaction.getbyid)
router.put(
  '/v1/pay/confirm/:bookingId',
  authorize,
  isAdmin,
  transaction.handleConfirmPaymentbookingid,
)
router.put(
  '/v1/pay/reject/:bookingId',
  authorize,
  isAdmin,
  transaction.handleRejectPaymentbookingid,
)
router.get('/v1/pay/find/detail/:bookingId', transaction.getDetailTransaction)

// notification
router.get(
  '/v1/notification/admin',
  authorize,
  isAdmin,
  notificationController.getNotificationAdmin,
)
router.get(
  '/v1/notification',
  authorize,
...
router.post(
  '/v1/wallet/topup/confirm/:walletHistoryId',
  authorize,
  eWalletController.topUpConfirmation,
)
router.post(
  '/v1/wallet/payment/:bookingId',
  authorize,
  eWalletController.paymentHandler,
)
router.get('/v1/wallet/history', authorize, eWalletController.getWalletHistory)
router.get(
  '/v1/wallet/history/:walletHistoryId',
  authorize,
  eWalletController.getDetailWalletHistory,
)
router.post('/v1/wallet/changepin', authorize, eWalletController.changePassword)
router.post(
  '/v1/wallet/resetpin',
  authorize,
  eWalletController.resetPINreqeuest,
  nodeMailer.sendResetPIN,
)
router.post(
  '/v1/wallet/resetpin/confirm',
  authorize,
  eWalletController.consfirmNewPin,
)
